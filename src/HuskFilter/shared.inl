/*
  ...
  ---------------------------------------------------------------------------
  VENDORERET OG BRANDET af Husk Webcam 2026-09-19.

  Filen er UnityCaptures egen og beholder sin oprindelige ophavsret og licens
  ORDRET. Resten af dette repo er ogsaa MIT, men ophavsretten til DENNE fil er
  Bernhard Schellings og MHD Yamen Sarajis - notitsen oeverst maa aldrig
  fjernes, omskrives eller erstattes af projektets egen.
  Upstream: https://github.com/schellingb/UnityCapture

  AENDRINGEN er ét sted og ét sted alene: de fire delte objekter har faaet
  HUSK-navne i stedet for UnityCapture-navne. Det er ikke kosmetik. En egen
  CLSID isolerer KUN COM-registreringen; deler to filtre stadig mutex, events
  og mapping, sender de til hinandens klienter - og den fejl ville ligne et
  flimrende billede frem for en navnekollision.

  Husets Unity Capture-installation (Python-vejen) kan derfor koere ved siden af
  uden at de to roerer hinanden.
  ---------------------------------------------------------------------------
*/
...
private:
	bool Open(bool ForReceiving)
	{
		...
		//BRANDET: egne navne, saa Husk Webcam og et evt. installeret Unity Capture ikke
		//deler delt hukommelse. Formen er upstreams: cifferet skrives paa plads
		//sizeof-2, og CapNum 0 bruger NUL-terminatoren for bagudkompatibilitet.
		char CS_NAME_MUTEX      [] = "HuskWebcam_Mutx0"; CS_NAME_MUTEX      [sizeof(CS_NAME_MUTEX      ) - 2] = CSCapNumChar;
		char CS_NAME_EVENT_WANT [] = "HuskWebcam_Want0"; CS_NAME_EVENT_WANT [sizeof(CS_NAME_EVENT_WANT ) - 2] = CSCapNumChar;
		char CS_NAME_EVENT_SENT [] = "HuskWebcam_Sent0"; CS_NAME_EVENT_SENT [sizeof(CS_NAME_EVENT_SENT ) - 2] = CSCapNumChar;
		char CS_NAME_SHARED_DATA[] = "HuskWebcam_Data0"; CS_NAME_SHARED_DATA[sizeof(CS_NAME_SHARED_DATA) - 2] = CSCapNumChar;
